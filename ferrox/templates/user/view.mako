...
<div class="basic-box TODO">
    ...
    <ul class="thumbnail-row">
        ...
        <li>${h.HTML.a(h.image_tag(h.url_for(controller='gallery', action='file', filename=submission.thumbnail.mogile_key), submission.title), href=h.url_for(controller='gallery', action='view', id=submission.id, username=c.user.username))}</li>
        % endfor
    </ul>
